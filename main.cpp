...
<body>
  <div class="container">
    ...
    server.Post("/admin/export-report", [&db](const httplib::Request&, httplib::Response& res) {
        ...
        auto writeMetric = [&](std::ostringstream& csv, const std::string& label, const std::string& sql) {
            ...
            if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
                if (sqlite3_step(stmt) == SQLITE_ROW) {
                    ...
                    value = text ? reinterpret_cast<const char*>(text) : "0";
                }
            }
    
            sqlite3_finalize(stmt);
            csv << csvEscape(label) << "," << csvEscape(value) << "\n";
        };
    
        auto writeQueryToCsv = [&](std::ostringstream& csv, const std::string& sql) {
            sqlite3_stmt* stmt = nullptr;
    
            if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) != SQLITE_OK) {
                csv << "SQL Error," << csvEscape(sqlite3_errmsg(db)) << "\n\n";
                return;
            }
            ...
            for (int i = 0; i < colCount; i++) {
                if (i > 0) csv << ",";
                csv << csvEscape(sqlite3_column_name(stmt, i));
            }
            csv << "\n";
            ...
            while (sqlite3_step(stmt) == SQLITE_ROW) {
                for (int i = 0; i < colCount; i++) {
                    ...
                    csv << csvEscape(text ? reinterpret_cast<const char*>(text) : "");
                }
                csv << "\n";
            }
        ...
        csv << "QueueSmart Report\n";
        csv << "Generated By," << csvEscape(getUsernameById(currentUserId)) << "\n";
        csv << "Report Format,CSV\n\n";
    
        csv << "QUEUE USAGE STATISTICS\n";
        csv << "Metric,Value\n";
        writeMetric(csv, "Total Queue Entries", "SELECT COUNT(*) FROM queue;");
        writeMetric(csv, "Currently Open Queue Entries", "SELECT COUNT(*) FROM queue WHERE status='open';");
        writeMetric(csv, "Average Wait Time", "SELECT ROUND(AVG(wait_time), 1) FROM queue;");
        writeMetric(csv, "Maximum Wait Time", "SELECT MAX(wait_time) FROM queue;");
        writeMetric(csv, "Total Services", "SELECT COUNT(*) FROM services;");
        writeMetric(csv, "Total Users", "SELECT COUNT(*) FROM accounts;");
        csv << "\n";
    
        csv << "SERVICE DETAILS AND QUEUE ACTIVITY\n";
        writeQueryToCsv(csv,
            "SELECT "
            "s.id AS service_id, "
            "s.name AS service_name, "
            "s.description AS description, "
            "s.estimated_service_time AS estimated_service_time_min, "
            "s.priority AS priority, "
            "s.length AS queue_length, "
            "COUNT(q.id) AS total_queue_entries, "
            "ROUND(AVG(q.wait_time), 1) AS average_wait_time "
            "FROM services s "
        ...
        csv << "USER CUSTOMER QUEUE PARTICIPATION HISTORY\n";
        writeQueryToCsv(csv,
            "SELECT "
            "q.id AS queue_id, "
        ...
        std::string csvData = csv.str();
    
        res.set_header("Content-Disposition", "attachment; filename=\"queue_report.csv\"");
        res.set_content(csvData, "text/csv");
    });
