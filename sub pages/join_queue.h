...
#include "queue_management.h"
    ...
    static inline std::string joinQueuePage() {
        ...
        for (const auto& svc : services) {
            ...
            // Clamp to [0, 120], map to hue [120, 0] (green -> red)
            ...
            // Derive a status badge from estimated wait
            ...
            queueCards +=
                ...
                "        <span class=\"char-count\" id=\"charCount-" + idStr + "\">0 / 128</span>"
                "      </div>"
                ...
                "        " + (statusClass == "status-full" ? "disabled" : "") + "></textarea>"
                ...
                "        " + (statusClass == "status-full" ? "disabled" : "") + ">"
                "        " + (statusClass == "status-full" ? "Queue Full" : "Join Queue") +
                "      </button>"
                "    </div>"
                "  </div>"
                "</div>";
